...
class MenuRepository:
    ...
    async def _create_menu(self, menu: menu_schemas.MenuCreate) -> menu_schemas.ShowMenu:
        try:
            ...
            raise HTTPException(status_code=404, detail='menu already exist')
        self.db.add(m)
    ...
    async def _update_menu(self, menu: menu_schemas.MenuCreate, api_test_menu_id: str) -> menu_schemas.ShowMenu:
        try:
            ...
            raise HTTPException(status_code=404, detail='menu not found')
        await self.db.execute(update(models.Menu).where(models.Menu.id == api_test_menu_id).values(
    ...
    async def _get_all_data(self) -> list:
